...
def create_ui():
    ...
    # Build the list of UI interfaces. In the stock webui this includes several tabs
    # such as txt2img, extras, png info, training, etc.  For our trimmed-down fork
    # we only expose the img2img tab.  All other interfaces remain loaded
    # internally, but they are not added to the visible Gradio tabs.  This keeps
    # functionality such as ControlNet and LoRA support intact while removing
    # unused UI components.
    interfaces = [
        (img2img_interface, "img2img", "img2img"),
    ]
    # You could optionally expose the Settings tab by uncommenting the next line:
    # interfaces.append((settings.interface, "Settings", "settings"))

    # Do not add any additional tabs from extensions or settings in this fork.
    # Disabling script_callbacks.ui_tabs_callback() prevents external scripts from
    # registering their own tabs.  Similarly, we skip adding the Settings and
    # Extensions tabs by not appending them here.
    # interfaces += script_callbacks.ui_tabs_callback()
    # interfaces += [(settings.interface, "Settings", "settings")]

    # Do not expose the built-in Extensions UI.
