...
class Toprow:
    ...
    def create_submit_box(self):
        with gr.Row(elem_id=f"{self.id_part}_generate_box", elem_classes=["generate-box"] + (["generate-box-compact"] if self.is_compact else []), render=not self.is_compact) as submit_box:
            ...
            # In the simplified img2img-only UI we hide the interrupt and skip buttons to reduce clutter.
            # They remain present for txt2img but are invisible for img2img.
            self.interrupt = gr.Button('Interrupt', elem_id=f"{self.id_part}_interrupt", elem_classes="generate-box-interrupt", tooltip="End generation immediately or after completing current batch", visible=not self.is_img2img)
            self.skip = gr.Button('Skip', elem_id=f"{self.id_part}_skip", elem_classes="generate-box-skip", tooltip="Stop generation of current batch and continues onto next batch", visible=not self.is_img2img)
            self.interrupting = gr.Button('Interrupting...', elem_id=f"{self.id_part}_interrupting", elem_classes="generate-box-interrupting", tooltip="Interrupting generation...", visible=not self.is_img2img)
            # Use a localized label for the generate button in img2img mode
            generate_label = 'Generovat obrázek' if self.is_img2img else 'Generate'
            self.submit = gr.Button(generate_label, elem_id=f"{self.id_part}_generate", variant='primary', tooltip="Right click generate forever menu")
